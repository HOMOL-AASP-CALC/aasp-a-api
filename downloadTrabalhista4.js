...
const BASE_OUTPUT_DIR = '/Users/mrozgrin/calculos_atualiza';
...
async function baixarPdf(idCalculo) {
  ...
  console.log(`Baixando PDF do cálculo ${idCalculo} de ${url}`);
  ...
  console.log(`PDF salvo em ${destinoArquivo}`);

  setTimeout(async () => {
    console.log(`Baixando HTML do cálculo ${idCalculo} de ${url}`);
    const resposta = await axios.get(urlHTML, { responseType: 'arraybuffer', timeout: 60000 });
    await fs.promises.writeFile(destinoArquivoHTML, resposta.data);
    console.log(`HTML salvo em ${destinoArquivoHTML}`);
  }, 3000);    

  setTimeout( () => {
    console.log(`Matando processo ${idCalculo} de ${urlMATA}`);
    axios.get(urlMATA, { responseType: 'arraybuffer', timeout: 60000 });
  }, 7000); 
...
async function executar() {
  ...
  try {
    const [registros] = await pool.query('SELECT id FROM lista_calculo WHERE tipo = ? ORDER BY id DESC limit 10', ['t4']);
